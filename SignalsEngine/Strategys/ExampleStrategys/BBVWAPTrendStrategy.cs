...
{
    public class BBVWAPTrendStrategy : ConditionStrategy
    {
        public int Period { get; set; }
        public float StdDevFactor { get; set; }
        public BBVWAPTrendStrategy(MarketInfo marketInfo, TimeFrames timeFrame)
        : base("Bolinger Bands VWAP Trend Strategy Example", marketInfo, timeFrame)
        {
            this.Period = 200;
            this.StdDevFactor = 2;
            AddConditions();
        ...
        public BBVWAPTrendStrategy(int Period, float StdDevFactor)
        : base(String.Format("Bolinger Bands {0}:{1} VWAP Trend Strategy Example", Period, StdDevFactor), null, TimeFrames.H1)
        {
            this.Period = Period;
