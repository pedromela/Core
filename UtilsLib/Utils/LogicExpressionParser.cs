...
namespace UtilsLib.Utils
{
    ...
    {
        bool Parse(LogicExpressionParser aParser, string aCommand, out ValueProvider aResult);
    }
    ...
    public class CompareCross : CompareStatement
    {
        protected double lastOp1 = 0;
        protected double lastOp2 = 0;
        protected override bool Compare(double aOp1, double aOp2) 
        {
            bool condition = (aOp1 > aOp2 && lastOp1 < lastOp2) || (aOp1 < aOp2 && lastOp1 > lastOp2);
            lastOp1 = aOp1;
            lastOp2 = aOp2;
            return condition;
        }
    ...
    public class CompareCrossUp : CompareStatement
    {
        protected double lastOp1 = 0;
        protected double lastOp2 = 0;
        protected override bool Compare(double aOp1, double aOp2)
        {
            bool condition = (aOp1 > aOp2 && lastOp1 < lastOp2);
            lastOp1 = aOp1;
            lastOp2 = aOp2;
            return condition;
        }
    ...
    public class CompareCrossDown : CompareStatement
    {
        protected double lastOp1 = 0;
        protected double lastOp2 = 0;
        protected override bool Compare(double aOp1, double aOp2)
    ...
    public class ParsingContext
    {
        ...
        private void SubstitudeCommand(LogicExpressionParser aParser, ref string aExpression, int aIndex)
        {
            int closing = FindClosingBracket(aExpression, aIndex, '{', '}');
            if (closing > aIndex + 1)
            {
        ...
        protected virtual ValueProvider ParseCommand(LogicExpressionParser aParser, string aCommand)
        {
        ...
        public virtual void Preprocess(LogicExpressionParser aParser, ref string aExpression)
        {
    ...
    public class LogicExpressionParser
    {
        ...
        public LogicExpressionParser() : this(new ParsingContext()) { }
        public LogicExpressionParser(ParsingContext aParsingContext)
        {
        ...
        private ILogicResult ParseLogicResult(string aExpression, int aMaxRecursion)
        {
            ...
            else if (aExpression.Contains(" cross "))
            {
                string[] parts = aExpression.Split(new string[] { " cross " }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 2)
                    throw new ParseException("cross operator needs an expression on either side");
                return new CompareCross { op1 = ParseNumber(parts[0].Trim(), aMaxRecursion), op2 = ParseNumber(parts[1].Trim(), aMaxRecursion) };
            }
            else if (aExpression.Contains(" crossup "))
            {
                string[] parts = aExpression.Split(new string[] { " crossup " }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 2)
                    throw new ParseException("crossup operator needs an expression on either side");
                return new CompareCrossUp { op1 = ParseNumber(parts[0].Trim(), aMaxRecursion), op2 = ParseNumber(parts[1].Trim(), aMaxRecursion) };
            }
            else if (aExpression.Contains(" crossdown "))
            {
                string[] parts = aExpression.Split(new string[] { " crossdown " }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 2)
                    throw new ParseException("crossdown operator needs an expression on either side");
                return new CompareCrossDown { op1 = ParseNumber(parts[0].Trim(), aMaxRecursion), op2 = ParseNumber(parts[1].Trim(), aMaxRecursion) };
            }
